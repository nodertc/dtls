...
module.exports = class AEADCipher extends Cipher {
  ...
  init(session) {
    ...
    const stream = createDecodeStream(keyBlock);
    ...
    debug('client iv', clientNonceImplicit);
    debug('server iv', serverNonceImplicit);
  ...
  encrypt(session, data, header) {
    ...
    const additionalDataStream = createEncodeStream();
    ...
    encode(additionalData, additionalDataStream, AEADAdditionalData);

    const cipher = crypto.createCipheriv(this.blockAlgorithm, writeKey, iv);

    cipher.setAAD(additionalDataStream.slice());
  ...
  decrypt(session, data, header) {
    ...
    decipher.setAAD(additionalDataStream.slice());
    decipher.setAuthTag(authTag);

    const headPart = decipher.update(encryted);
    const finalPart = decipher.final();

    return finalPart.length ? Buffer.concat([headPart, finalPart]) : headPart;
  }
