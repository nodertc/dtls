...
const { createMachine, createState: createFlow } = require('next-state');
...
const transitions = {
  [null]: createFlow(CLIENT_HELLO),
  [CLIENT_HELLO]: createFlow(HELLO_VERIFY_REQUEST, SERVER_HELLO), // default state
  [HELLO_VERIFY_REQUEST]: createFlow(CLIENT_HELLO),
  [SERVER_HELLO]: createFlow(
    CERTIFICATE,
    SERVER_KEY_EXCHANGE,
    SERVER_HELLO_DONE
  ),
  [CERTIFICATE]: createFlow(
    SERVER_KEY_EXCHANGE,
    CERTIFICATE_REQUEST,
    SERVER_HELLO_DONE
  ),
  [CLIENT_CERTIFICATE]: createFlow(
    CLIENT_KEY_EXCHANGE,
    CLIENT_CHANGE_CIPHER_SPEC
  ),
  [SERVER_KEY_EXCHANGE]: createFlow(CERTIFICATE_REQUEST, SERVER_HELLO_DONE),
  [CERTIFICATE_REQUEST]: createFlow(SERVER_HELLO_DONE),
  [SERVER_HELLO_DONE]: createFlow(CLIENT_KEY_EXCHANGE, CLIENT_CERTIFICATE),
  [CLIENT_KEY_EXCHANGE]: createFlow(
    CERTIFICATE_VERIFY,
    CLIENT_CHANGE_CIPHER_SPEC
  ),
  [CERTIFICATE_VERIFY]: createFlow(CLIENT_CHANGE_CIPHER_SPEC),
  [CHANGE_CIPHER_SPEC]: createFlow(FINISHED),
  [CLIENT_CHANGE_CIPHER_SPEC]: createFlow(CLIENT_FINISHED),
  [FINISHED]: null,
  [CLIENT_FINISHED]: createFlow(CHANGE_CIPHER_SPEC),
};
...
class ClientProtocol extends Writable {
  ...
  constructor(session) {
    ...
    const state = createMachine(transitions);
    this[_state] = state;
