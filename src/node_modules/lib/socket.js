...
class Socket extends Duplex {
  ...
  constructor(options = {}) {
    ...
    if (Buffer.isBuffer(options.pskIdentity)) {
      validatePSKIdentity(options.pskIdentity.toString());
    ...
    if (Buffer.isBuffer(options.pskSecret)) {
      ...
      validatePSKSecret(session.clientPSKSecret);
...
function validatePSKSecret(pskSecret) {
  if (pskSecret.length > 64 || pskSecret.length === 0) {
...
function validatePSKIdentity(pskIdentity) {
  if (pskIdentity.length > 128 || pskIdentity.length === 0) {
