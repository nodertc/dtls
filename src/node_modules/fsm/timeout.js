 ...
 */
class StateMachine extends Readable {
  /**
   * @constructor
   ...
   * @param {bool} isLastFlight check if queue contains last flight
   */
  send(isLastFlight = false) {
    this[_next](SENDING);

    this[_queue].forEach(chunk => this.push(chunk));
  ...
  [_next](state) {
    ...
    if (!transition.has(state)) {
      throw new Error(`Invalid state change from ${this.state} to ${state}`);
    }
...
Object.defineProperty(StateMachine.prototype, 'readableBuffer', {
  // Getter `readableBuffer` was added in v9.4.0.
  // Make it compatible with v8.
  // @see https://github.com/nodejs/node/commit/e20af3371bb1f0ee16795528cbcbc674408842cb
  enumerable: false,
  get() {
 ...
 */
function createClientStateMachine() {
  return new StateMachine(PREPARING);
}
 ...
 * @returns {StateMachine}
 */
function createServerStateMachine() {
  return new StateMachine(WAITING);
}

module.exports = {
  createClientStateMachine,
  createServerStateMachine,
  StateMachine,
  states: {
    PREPARING,
    SENDING,
