...
module.exports = class Protocol12ReaderClient extends Writable {
  ...
  [_serverKeyExchange]() {
    ...
    if (this.session.nextCipher.kx.keyType === keyTypes.RSA) {
      assert(digitalSign.algorithm.sign === signAlgoSignTypes.RSA);
      assert(digitalSign.algorithm.hash === signAlgoHashTypes.SHA1);
    }

    if (this.session.nextCipher.kx.keyType === keyTypes.ECDSA) {
      assert(digitalSign.algorithm.sign === signAlgoSignTypes.ECDSA);
    }

    const verifier = crypto.createVerify(
      getHashNameBySignAlgo(digitalSign.algorithm.hash)
    );
