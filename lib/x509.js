...
function getPublicKeyPem(data) {
  ...
  const publicKeyInfo = data.tbsCertificate.subjectPublicKeyInfo
  const publicKey = SubjectPublicKeyInfo.encode(publicKeyInfo, 'der')

  return pem.encode(publicKey, pem.PUBLIC_KEY)
}
